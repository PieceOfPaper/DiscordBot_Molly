...
## GPT 리뷰 전달 규칙
- GPT의 저장소·Google Sheets 검토 결과는 `docs/reviews/`에서 관리하며, 세부 절차와 상태 정의는 `docs/reviews/README.md`를 따른다.
- 개발을 시작하기 전에 현재 요청과 관련된 `docs/reviews/pending/` 문서를 확인한다.
- 리뷰 문서의 `검토안`과 `보류` 항목은 사용자의 판단 전 제안이므로 자동으로 구현하지 않는다.
- 사용자가 현재 요청에서 “검토 내용 반영”, “필수 수정 적용”처럼 대상과 범위를 명시한 경우에만 해당 범위를 승인된 작업으로 본다. 범위가 모호하면 구현 전에 질문한다.
- 승인된 리뷰를 구현할 때는 리뷰의 기준 커밋 이후 변경 사항을 먼저 확인하고, Google Sheets 데이터가 관련되면 최신 시트를 다시 읽는다.
- 현재 사용자 요청, 승인된 리뷰, 배틀 기획서, 코드·시트가 서로 충돌하면 임의로 결정하지 않고 충돌 내용과 선택지를 보고한다.
- 구현 후 리뷰 문서의 `구현 기록`에 변경 파일과 시트, 실행한 검증, 미반영 항목을 기록한다. 모든 승인 항목을 완료하면 상태를 `완료`로 바꾸고 `docs/reviews/completed/`로 이동한다.
- 리뷰 반영으로 확정 설계가 달라졌다면 관련 기획서와 데이터 설명도 함께 갱신한다.
