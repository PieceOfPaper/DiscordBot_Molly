...
- 슬래시 명령을 추가·변경·삭제할 때는 README.md의 `슬래시 커맨드` 표도 함께 갱신한다. 명령어, 설명, 옵션의 필수 여부·기본값·제한을 실제 코드와 일치시킨다. `/도움말` 분류(`Commands/HelpCatalog.cs`)에도 반영한다.
- `/도움말`은 전체 명령 목록(`HelpCatalog.Categories`)과 컨텐츠별 상세 도움말(`HelpCatalog.Topics`)로 나뉜다. 규칙·진행 순서가 있는 놀이나 옵션·알림 조건 설명이 필요한 기능을 추가하면 상세 도움말 주제를 새로 만들거나 기존 주제에 넣는다. 한 줄 요약으로 충분한 단순 명령은 목록에만 넣는다.
  - 주제는 명령 하나가 아니라 컨텐츠 단위로 묶는다(예: 배틀 = `/캐릭터등록`·`/배틀`·`/배틀종료`). `Commands`에 관련 명령을 모두 적는다.
  - 새 주제는 `HelpTopic`을 추가하고 `Commands/HelpCommand.cs`의 `주제` Choice에 같은 이름·순서로 추가한다. README `/도움말` 행의 주제 목록도 갱신한다.
  - 게임 규칙·시간·기본값·알림 조건 등 동작을 바꾸면 해당 주제의 본문도 실제 코드와 맞게 고친다. 본문은 사용자 대상 한국어로 쓰고 필드 1024자·임베드 전체 길이 제한을 지킨다(회귀 테스트가 검사).
...
- 구현 에이전트(Codex, Claude Code)는 Issue를 닫지 않는다. 승인 범위의 구현·검증·커밋이 끝나면 커밋 해시와 재검토 요청 범위를 댓글로 남기고 `GPT 재검토 대기` 상태로 둔다. Issue는 GPT가 재검토한 뒤 닫는다.
- 일부 항목이 보류되었으면 본문이나 댓글에 남기고 필요하면 후속 Issue로 분리한다.
