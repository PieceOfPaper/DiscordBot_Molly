...
`/스피드퀴즈`는 `문제종목`, `문제수`(기본 10), `제한시간`(기본 30초)을 받아 길드 안에 `몰리퀴즈-날짜시간` 채널을 만들고 진행합니다. 문제종목은 `시즌2룬효과로이름`, `시즌2무기룬효과로이름`, `시즌2방어구룬효과로이름`, `시즌2앰블럼룬효과로이름`을 지원합니다. 시즌 2에서 해당 분류의 이름·효과가 모두 있는 룬을 중복 없이 무작위로 출제하며, 요청한 문제수보다 데이터가 적으면 가능한 수만 출제합니다. 룬 이름 끝의 `+`는 정답에서 제거합니다. 준비 30초와 문제 사이 10초는 코드의 상수로 지정되어 있습니다. 길드당 한 게임만 진행하며, 정답은 띄어쓰기와 `+` 제거 및 영문 소문자 변환 후 비교합니다. 문제 중에는 1초 간격으로 남은 시간을 갱신하고 마지막 3초는 `3·2·1`로 표시하며, 정답 시 소요 시간도 표시합니다.

`/스피드퀴즈종료`를 사용하면 해당 길드의 진행 중인 게임을 즉시 종료하고, 퀴즈 채널에 지금까지의 점수와 현재 우승자를 최종 결과로 표시합니다.

`자음표시`를 `true`로 선택하면 문제 Embed에 초성 힌트를 함께 표시합니다. `거대한 분노+`는 `ㄱㄷㅎ ㅂㄴ`로 표시되며, 띄어쓰기는 유지하고 `+` 같은 기호는 제외합니다. 시간 갱신 중에도 힌트는 유지됩니다.
