...
| `/진혹거퀴즈` | 전용 채널에서 룬 이름·효과가 진실인지 거짓인지 이모지로 맞히기 시작 | `문제종목`(필수, 시즌2룬·무기·방어구·앰블럼·장신구·장신구 제외 중 선택), `문제수`(최소 1, 기본 10, 출제 가능 수로 자동 조정), `제한시간`(문제당 초, 최소 1, 기본 30) |
| `/눈치게임` | 대상자를 부른 뒤 현재 채널에서 즉시 숫자 눈치게임 시작 | `대상자`(필수 텍스트: 함께 할 사용자를 모두 멘션, 봇 제외; 실행자는 자동 참가), `제한시간`(초, 1~120, 기본 30). 참가자들은 한 번씩 `1`부터 외치며, N명일 때 N-1번째 정상 숫자 뒤 아직 외치지 않은 사람이 걸림 |
| `/당첨뽑기` | 멘션한 대상 중 당첨자를 뽑고, 참가자가 버튼으로 각자의 결과를 공개 | `대상자`(필수 텍스트: 참가할 사용자를 모두 멘션, 봇 제외; 실행자는 자동 참가하지 않음), `당첨수`(최소 1, 기본 1, 참가자 수 이하), `제한시간`(초, 최소 0, 기본 60; 0이면 즉시 전체 결과 공개). 제한시간 뒤에는 아직 결과를 확인하지 않은 참가자의 결과를 자동 공개 |
| `/당첨뽑기종료` | 현재 길드의 당첨뽑기를 즉시 종료하고 아직 공개되지 않은 결과 표시 | 없음 |
| `/라이어게임` | 몰리 놀이터 전용 채널에서 참가형 라이어게임 시작 | 없음. 참가 버튼을 30초간 받고, 최소 3명·최대 20명으로 진행 |
| `/놀이종료` | 현재 길드의 진행 중인 스피드퀴즈 또는 자음퀴즈를 강제 종료하고 최종 결과 표시 | 없음 |
...
`/진혹거퀴즈`는 `문제종목`, `문제수`(기본 10), `제한시간`(기본 30초)을 받아 같은 방식으로 전용 채널에서 진행합니다. 문제 메시지에는 ✅(진실)와 ❌(거짓) 버튼을 표시하며, 참가자는 문제마다 첫 버튼 선택 하나만 할 수 있습니다. 제한시간 종료 시 정답 버튼을 고른 모든 참가자에게 1점씩 부여합니다. 룬 문제는 이름과 효과가 실제로 일치하면 진실, 같은 분류 안의 다른 룬 효과를 조합하면 거짓입니다. 따라서 무기·방어구·앰블럼·장신구는 물론 `시즌2룬`과 `시즌2장신구룬빼고` 종목도 이름의 분류 밖 효과를 섞지 않습니다. 문제마다 결과·현재 점수를 표시하고 마지막에 우승자 발표 후 채널을 잠급니다.

스피드퀴즈·자음퀴즈·진혹거퀴즈는 길드마다 합쳐서 하나만 준비 또는 진행할 수 있습니다. `/놀이종료`를 사용하면 해당 길드의 진행 중인 게임을 즉시 종료하고, 몰리 놀이터 채널에 지금까지의 점수와 현재 우승자를 최종 결과로 표시합니다. 종료 시 채널 이름에 `-종료`를 붙이고, 퀴즈를 시작한 채널로 돌아갈 수 있는 링크와 30초 뒤 채널이 보기 전용으로 잠긴다는 안내를 표시합니다. 잠기기 전까지 5초 단위로 남은 시간을 알린 뒤 `@everyone`의 메시지 전송·반응·스레드·슬래시 명령 등 상호작용 권한을 차단합니다.
...
스피드퀴즈는 일반 메시지를 정답으로 받으므로 Discord Developer Portal에서 **Message Content Intent**를 켜야 합니다. 봇에는 채널 관리, 채널 보기, 메시지 보내기 권한도 필요합니다. `/진혹거퀴즈`는 버튼 방식이라 반응 추가나 메시지 기록 보기 권한이 추가로 필요하지 않습니다.
