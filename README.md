...
| `/배틀` | 등록한 캐릭터로 같은 서버 사용자를 상대로 공개 스레드에서 무조작 자동전투 시작 | `상대`(필수 사용자 멘션). 두 사용자 모두 `/캐릭터등록` 필요, 시작 시 두 참가자를 멘션(캐릭터명)으로 표시하고 30초 준비 후 턴마다 5초 간격으로 진행. 최근 랭킹 스냅샷은 1시간 동안 사용 |
