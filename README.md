...
| `/스피드퀴즈` | 전용 채널에서 룬 효과로 이름 또는 장신구 룬 이름으로 클래스 맞히기 시작 | `문제종목`(필수, 시즌2 전체·무기·방어구·앰블럼·장신구·장신구 제외 효과→이름 또는 장신구 이름→클래스 중 선택), `문제수`(최소 1, 기본 10, 출제 가능 수로 자동 조정), `제한시간`(문제당 초, 최소 1, 기본 30), `자음표시`(bool, 기본 false) |
| `/자음퀴즈` | 전용 채널에서 자음·종류·힌트를 보고 룬/NPC/클래스/지역 이름 맞히기 시작 | `문제종목`(필수, 시즌2룬·무기·방어구·앰블럼·장신구·장신구 제외·NPC·클래스·지역·NPC+클래스+지역 중 선택), `문제수`(최소 1, 기본 10, 출제 가능 수로 자동 조정), `제한시간`(문제당 초, 최소 1, 기본 30) |
| `/퀴즈종료` | 현재 길드의 진행 중인 스피드퀴즈 또는 자음퀴즈를 강제 종료하고 최종 결과 표시 | 없음 |
...
스피드퀴즈와 자음퀴즈는 길드마다 합쳐서 하나만 준비 또는 진행할 수 있습니다. `/퀴즈종료`를 사용하면 해당 길드의 진행 중인 게임을 즉시 종료하고, 몰리 놀이터 채널에 지금까지의 점수와 현재 우승자를 최종 결과로 표시합니다. 종료 시 채널 이름에 `-종료`를 붙이고, 퀴즈를 시작한 채널로 돌아갈 수 있는 링크와 30초 뒤 채널이 보기 전용으로 잠긴다는 안내를 표시합니다. 잠기기 전까지 5초 단위로 남은 시간을 알린 뒤 `@everyone`의 메시지 전송·반응·스레드·슬래시 명령 등 상호작용 권한을 차단합니다.
...
스피드퀴즈는 일반 메시지를 정답으로 받으므로 Discord Developer Portal에서 **Message Content Intent**를 켜야 합니다. 봇에는 채널 관리, 채널 보기, 메시지 보내기 권한도 필요합니다.
