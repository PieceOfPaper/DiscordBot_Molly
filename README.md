...
스피드퀴즈와 자음퀴즈는 길드마다 합쳐서 하나만 준비 또는 진행할 수 있습니다. `/퀴즈종료`를 사용하면 해당 길드의 진행 중인 게임을 즉시 종료하고, 퀴즈 채널에 지금까지의 점수와 현재 우승자를 최종 결과로 표시합니다. 퀴즈 종료 시 채널 이름에 `-종료`를 붙이고, 퀴즈를 시작한 채널로 돌아갈 수 있는 링크와 30초 뒤 채널이 보기 전용으로 잠긴다는 안내를 표시합니다. 잠기기 전까지 5초 단위로 남은 시간을 알린 뒤 `@everyone`의 메시지 전송·반응·스레드·슬래시 명령 등 상호작용 권한을 차단합니다.
