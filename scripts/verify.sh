...
class_emoji_count=$(find artifacts/publish/assets/class_emojis -name '*.png' -size +0 | wc -l | tr -d ' ')
[[ "$class_emoji_count" -eq 21 ]] || { echo "Missing publish class emojis: $class_emoji_count/21" >&2; exit 1; }
